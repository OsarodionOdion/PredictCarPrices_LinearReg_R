...
> The result of the scatter plot shows a slightly negative linear association between Price and Mileage
...
> The records with the 10 highest 'Price' show a negative linear association between 'Price' and 'Mileage'. This is also noticeable in the scatter plot, as depicted by the outliers.
...
**QUESTION** The MAE on test is less than the MAE on train. Is this expected or unexpected and why? It is expected because the mean price for the test is less than that of the train. The MAE test value is dependent on the magnitude and frequency of the price values. Since the partition was unequal and random, the magnitude and frequency of the values in both data sets are unequal, therefore their MAEs from the mean will differ. The test is the lesser set which could result in the MAE being less than the train. 
...
The results are not entirely surprising. Though the R-Squared value is significantly low, I think it is worth keeping mileage in our model because though the data does not provide a perfect fit, this does not rid it off its ability to predict prices.
...
price_lm2 <- lm(Price ~ Cylinder, data = ucars_train)
price_lm3 <- lm(Price ~ Liter, data = ucars_train)
price_lm4 <- lm(Price ~ Doors, data = ucars_train)
...
From the linear models created, the p-values and std errors for price_lm2 and price_lm3 indicate them as better predictors than the other models. Also, both models have better R-Squared values than the others. Which indicate a better fit than the other models
...
With lower MAE values, price_lm2 and price_lm3 performed better than the other models
...
From the MAE test on the predicted models, we can infer that price_lm2 and price_lm3 are better than the other models. This indicates Cylinder and Liter as better predictors of Price than mileage and doors.
...
> Based on the scatter plots, the 3rd linear model (price_lm3) looks the best. The abline added to the plots provides a reference to measure the performance of each model. The abline for price_lm3 is closest to 45 degrees. 
